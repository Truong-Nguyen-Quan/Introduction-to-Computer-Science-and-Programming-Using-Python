...
def deal_hand(n):
    ...
    for i in range(num_vowels):
        if i == num_vowels - 1 and '*'not in hand.keys():
            hand['*'] = 1
            break
        x = random.choice(VOWELS)
        while x == '*' and x in hand.keys():
            x = random.choice(VOWELS)
...
def update_hand(hand, word):
    ...
    word = word.lower()
    for char in word:
        if char in new_hand.keys() and new_hand.get(char, 0) > 0:
            new_hand[char] -= 1
            if new_hand[char] == 0:
                del new_hand[char]
    return new_hand
...
def is_valid_word(word, hand, word_list):
    ...
    for char in word:
        if char not in hand.keys() or freq[char] > hand[char]:
            return False
    return True
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    total_score = 0
    while (True):
        if (calculate_handlen(hand) > 0) == False:
            print()
        ...
        if word_input == "!!":
            ...
            if is_valid_word(word_input, hand, word_list):
                total_score += get_word_score(word_input, calculate_handlen(hand))
                print('"' + word_input + '"' + ' earned', get_word_score(word_input, calculate_handlen(hand)), 'points. Total:', total_score, 'points')
            else:
    ...
    * Returns the total score for the series of hands
    ...
    while (True):
        try:
            ...
            print("That's not a valid number.")
    # hand1 = {'a': 1, 'c': 1, 'i': 1, 'p': 1, '*': 1, 't': 1, 'r': 1}
    # hand2 = {'d': 2, 'l': 1, 'o': 1, '*': 1, 't': 1, 'u': 1}
    for i in range(total_hands):
        ...
        while (True):
            ...
            if is_substitute == 'yes' or is_substitute == 'no':
                break
